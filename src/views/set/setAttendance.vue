    ...
    <div class="com-page-box">
      ...
      <div class="com-page-con">
        <div class="history-search">
          ...
          <span class="search-item">
               <el-button type="warning" icon="el-icon-mouse" @click="staffAttendanceaPassAll()">一键满勤</el-button>
              </span>
        </div>
        <div class="table">
          <el-table
              ...
              <el-table-column
                ...
                <template slot-scope="scope">
                  <i v-if="getAttendanceStatus(item, scope.row.monthRecords) == 1" class="el-icon-success"
                     style="color: #67C23A" @click="staffAttendanceaPass(item, scope.row)"></i>
                  <i v-if="getAttendanceStatus(item, scope.row.monthRecords) == 4" class="el-icon-remove"
                     style="color:#F56C6C" @click="staffAttendanceaPass(item, scope.row)"></i>
                  <i v-if="getAttendanceStatus(item, scope.row.monthRecords) == 5" class="el-icon-error"
                     style="color: #E6A23C" @click="staffAttendanceaPass(item, scope.row)"></i>
                  <i
                    v-if="getAttendanceStatus(item, scope.row.monthRecords) == 2 || getAttendanceStatus(item, scope.row.monthRecords) == 3"
                    class="el-icon-alarm-clock" style="color: #333333"
                    @click="staffAttendanceaPass(item, scope.row)"></i>
                </template>
              </el-table-column>
            </el-table-column>
          </el-table>
...
<script>
  ...
  import { getFirstDay, getLastDay } from '../../utils/utils'
  ...
  export default {
    ...
    computed: {},
    methods: {
      staffAttendanceaPassAll () {
        this.$confirm('确定一键满勤, 是否继续?', '提示', {
          ...
          API.attendance.staffAttendanceaPass({
            personIds: personIds,
      ...
      staffAttendanceaPass (item, row) {
        this.$confirm('确定设为已签到, 是否继续?', '提示', {
          confirmButtonText: '确定',
          cancelButtonText: '取消',
          type: 'warning',
        }).then(() => {
          API.attendance.singlePass({
            personIds: row.personId,
            attendanceDate: `${row.year}-${row.month}-${item}`,
          }).then(da => {
            if (da.status) {
              this.$message({
                type: 'success',
