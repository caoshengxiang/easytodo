...
export function getFirstDay (timeS) {
  ...
  data.setSeconds(0)
  data.setMinutes(0)
  let firstDate = parseInt(data.getTime() / 1000)
  return firstDate * 1000
}
 ...
 * @param month 月份
 **/
export function getLastDay (timeS) {
  if (!timeS) {
    return [null, null]
  }
  let data1 = new Date(timeS) // 下月
  if (data1.getMonth() === 12) {
    data1.setMonth(1)
  } else {
  ...
  data1.setSeconds(0)
  data1.setMinutes(0)
  let lastDate = parseInt(data1.getTime() / 1000) - 1
 ...
 * 一维数组拆分二维数组
 * @param array 拆分得数组
 * @param subGroupLength 长度进行拆分
 **/
export function groupedArray (array, subGroupLength) {
  let index = 0
  let newArray = []
